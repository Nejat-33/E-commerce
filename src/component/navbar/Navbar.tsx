import { faAddressCard, faCartArrowDown, faCartShopping, faCreditCard, faDesktop, faMoon, faSearch, faSun, faUserCircle } from "@fortawesome/free-solid-svg-icons"
...
const Navbar = () => {
  ...
  }, [isopen, isAccountopen, ismenuopen])


  useEffect(() => {
    setprev(path.pathname)
  }, [path.pathname])
    ...
    <div className="flex justify-between items-center  w-full px-4 py-3 bg-[#F9FAFB] dark:bg-slate-900  dark:border-slate-800">
      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        {!user && (
          ...
          <Link to={checkifuserlogin() ? "/wishlist" : "/login"} className="hidden lg:block text-gray-600 dark:text-gray-300 hover:text-red-500 transition-colors">
            <Heart size={24} />
          </Link>
        )}
        ...
        {user ?
          <div>
            <div onClick={() => setisAccountupen(!isAccountopen)} className="relative hidden md:flex items-center gap-2 cursor-pointer group">
              ...
              <div className="text-left leading-tight">
                <p className="text-[10px] text-gray-500">Hi {user.email}</p>
                <button className="text-[12px] font-bold dark:text-white block ">Account</button>
              </div>

              {isAccountopen && (
                <div ref={accountref} className="absolute z-30 top-8  -left-7 text-sm px-2 py-7 w-50 bg-white border border-gray-100 rounded-xl shadow-2xl flex flex-col gap-5">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <span onClick={() => { setisAccountupen(false); navigate('/userpage', { state: { tab: 'order' } }) }} className="flex gap-2  items-center rounded-lg hover:bg-indigo-50 py-3  w-full px-2"><FontAwesomeIcon icon={faBars} />My Order</span>
                    <span onClick={() => { setisAccountupen(false); navigate('/userpage', { state: { tab: 'wishlist' } }) }} className="flex gap-2  items-center rounded-lg hover:bg-indigo-50 py-3  w-full px-2"><Heart size={18} />Wishlist</span>
                    <span onClick={() => { setisAccountupen(false); navigate('/userpage', { state: { tab: 'payment' } }) }} className="flex gap-2  items-center rounded-lg hover:bg-indigo-50 py-3  w-full px-2"><FontAwesomeIcon icon={faCreditCard} />Payment</span>
                    <span onClick={() => { setisAccountupen(false); navigate('/userpage', { state: { tab: 'profile' } }) }} className="flex gap-2  items-center rounded-lg hover:bg-indigo-50 py-3  w-full px-2"><User size={18} />Profile Setting</span>
                    <span onClick={() => { setisAccountupen(false); navigate('/userpage', { state: { tab: 'address' } }) }} className="flex gap-2  items-center rounded-lg hover:bg-indigo-50 py-3  w-full px-2"><FontAwesomeIcon icon={faAddressCard} />Shipping Address</span>
                    <span onClick={logout} className="flex  gap-2 px-2  text-red-600"><LogOut size={18} className="hover:translate-x-1 transition-all duration-700" />Sign Out</span>
                  </div>
                </div>
              )}
          ...
          <div className="hidden md:flex items-center gap-2 cursor-pointer group">
            <span className="text-xl text-gray-600 dark:text-gray-300 group-hover:text-indigo-600"><FontAwesomeIcon icon={faUser} /></span>
            <div className="text-left leading-tight">
              <p className="text-[10px] text-gray-500">Welcome</p>
              <button className="text-[12px] font-bold dark:text-white block "><Link to='/login' className="hover:text-indigo-600">Login</Link>/ <Link to='/registration' className="hover:text-indigo-600">Signup</Link></button>
            </div>
          </div>
        }
